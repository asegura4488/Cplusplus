...
double Energia(double q1, double p1);
...
int main(int argc, char **argv){
  ...
  char ArchSalida5[] = "salida_5.dat";
  salida_5 = fopen(ArchSalida5, "w");
  ...
  int j;
  int k; 
  for (j=1;j<=4;j++) // condiciones iniciales del cuerpo 1
    {
      
      
      for (k=0;k<=100; k++) // cambiando condiciones iniciales del cuerpo 3
	{
	  double T=3E3;
	  double delta_t=5E-3;
	  ...
	  for(i=0;i<n_step;i++){ 
            ...
            energia_RK4 = Energia(q1_RK4, p1_RK4);
	    energia_LF4 = Energia(q1_LF, p1_LF);	    
	    fprintf(salida_5,"%f %.15e %.15e \n", t, energia_RK4, energia_LF4);	
